...
#define T_MESSAGE_US 1000000  // 1 second right now (should be around 1/15 of a second)
...
void check_shutdown_errors() {
  if (get_driver_eStop() || get_external_eStop() || get_crash() || get_door() || get_imd_status() || get_mcu_stat_fdbk() || get_door_lim_out() || get_mcu_latch()) {
    set_mcu_hv_en(0);
  }
}
